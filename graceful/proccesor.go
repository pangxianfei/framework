...
func closeQueue(quietly bool) {
	...
	if err := queue.Queue().Close(); err != nil {
		logFatal(quietly, "queue close failed", tmaic.V{"error": err})
	}
...
func closeCache(quietly bool) {
	...
	if err := cache.Cache().Close(); err != nil {
		logFatal(quietly, "cache close failed", tmaic.V{"error": err})
	}
...
func closeMonitor(quietly bool) {
	...
	if err := monitor.Shutdown(); err != nil {
		logFatal(quietly, "monitor close failed", tmaic.V{"error": err})
	}
